...
export const getAdminAccessContext = cache(async (): Promise<AdminAccessContext | null> => {
  ...
  const { data: dealership, error: dealershipError } = await supabase
    .from("dealerships")
    .select(
      "id, contact_email, contact_phone, created_at, default_financing_apr_percent, default_financing_headline, default_post_location_tag, default_sale_inclusions, facebook_page_url, logo_url, name, slug, updated_at, vehicle_catalog",
    )
